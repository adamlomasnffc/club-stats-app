...
def render_subtab_dashboard(team_key, has_match_center=True):
    if team_key == "Penguins":
        current_logo = CLUB_LOGO_URL
    elif team_key == "Socials":
        current_logo = SOCIALS_LOGO_URL
    elif team_key == "Community":
        current_logo = COMMUNITY_LOGO_URL
    else:
        current_logo = ""

    if has_match_center:
        tabs = [
            (current_logo, "Player Stats", True),
            ("📅", "Results", False),
            ("⚽", "Match Center", False),
            ("📰", "News", False)
        ]
    ...
    tiles = ""
    for icon, tab_name, is_img in tabs:
        ...
        if is_img:
            icon_html = f'<img src="{icon}" class="dash-img-icon" alt="{tab_name}">'
        else:
            icon_html = icon
